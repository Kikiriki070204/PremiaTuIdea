...
export class IdeasAceptadasAdminComponent implements OnInit {
  allIdeas: any[] = [];
  filteredIdeas: any[] = [];
  clientPage = 1;
  ...
  loadAllData(): void {
    ...
    (this.ideaService.ideasByStatusAndCategory(2, this.selectedCategoria, 1, this.selectedArea) as any).subscribe({
      next: (firstRes: any) => {
        ...
        for (let p = 2; p <= lastPage; p++) {
          requests.push(
            (this.ideaService.ideasByStatusAndCategory(2, this.selectedCategoria, p, this.selectedArea) as any)
